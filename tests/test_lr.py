...
from lir.calibration import FractionCalibrator, ScalingCalibrator, KDECalibrator, FourParameterLogisticCalibrator, \
    LogitCalibrator, LogitCalibratorInProbabilityDomain, KDECalibratorInProbabilityDomain
...
class TestLR(unittest.TestCase):
    ...
    def test_calibrated_scorer_with_distance_function(self):
        ...
        calibrated_scorer = CalibratedScorer(paired_manhattan_distances, KDECalibratorInProbabilityDomain())
        calibrated_scorer.fit(X, y)
        self.assertAlmostEqual(0, metrics.cllr(calibrated_scorer.predict_lr(X), y), places=3)

        calibrated_scorer = CalibratedScorerCV(paired_manhattan_distances, KDECalibratorInProbabilityDomain(), n_splits=5)
        calibrated_scorer.fit(X, y)
        self.assertAlmostEqual(0, metrics.cllr(calibrated_scorer.predict_lr(X), y), places=3)
